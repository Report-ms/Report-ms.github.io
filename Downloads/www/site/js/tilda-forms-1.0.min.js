...
window.tildaForm.send = function(formNode, btnSubmitNode, formType, formKey) {
    ...
    2 === formType || !formType && formKey) {
        ...
        var url = form.getAttribute("data-request-url")
        return xhr.open("POST", url, !0),
                        ...
                        if ("object" == typeof objData) {
                            ...
                            try {
                                if (objData && objData.redirectto && 3 == objData.redirectto.length && window.tildaForm.endpoint.substring(window.tildaForm.endpoint.length - 3) != objData.redirectto)
                                    return window.tildaForm.endpoint = "forms.tildaapi." + objData.redirectto,
                ...
                if (xhr.status >= 200 && xhr.status < 400) {
                    ...
                    o = xhr.responseText)
                        if ("{" == o.substring(0, 1)) {
                            var n = JSON.parse(o);
                            if ("object" == typeof n)
                                if (n && n.message && "OK" !== n.message)
                                    t.innerHTML = n.message;
                                else if (n && n.error) {
                                    var a, i = (a = t_forms__getErrorContainers(form, "Unknown error. Please, try again later.")).errorBoxes, s = a.allError;
                    ...
                    var l = "/tilda/" + formId + "/submitted/"
                      , c = "Send data from form " + formId;
                    window.Tilda && "function" == typeof window.Tilda.sendEventToStatistics ? window.Tilda.sendEventToStatistics(l, c, "", 0) : "undefined" != typeof ga && "tilda" !== window.mainTracker ? ga("send", {
                        hitType: "pageview",
                        page: l,
                        title: c
