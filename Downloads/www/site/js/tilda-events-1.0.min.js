...
function() {
    function t_events__initEvent() {
        ...
        function linkClickCreateEvent(e) {
            ...
            if (e.target && targetEl) {
                ...
                if (!virtPage) {
                    var record = targetEl.closest(".r"), recordId;
                    virtPage = "/tilda/click/" + (record ? record.getAttribute("id") : "") + "/?url=" + url
                }
                if (Tilda.sendEventToStatistics(virtPage, virtTitle),
                "http" === url.substring(0, 4) && -1 === url.indexOf(".typeform.com/"))
    ...
    Tilda.sendEcommerceEvent = function(type, productsArr) {
        if (void 0 === productsArr || 0 === productsArr.length)
            return !1;
        if (void 0 === type || "add" !== type && "remove" !== type && "purchase" !== type && "detail" !== type)
            return !1;
        for (var virtPage, virtTitle = "", virtPrice = 0, virtProducts = [], variant_str, productObj, recid = "", uid = "", lid = "", i = 0; i < productsArr.length; i++) {
            ...
            variant_str = "",
            void 0 !== productObj.options && productObj.options.length > 0 && Array.prototype.forEach.call(productObj.options, (function(option) {
            ...
            productObj.id && productObj.id > 0 && (id = productObj.id,
            virtProduct.id = productObj.id),
            productObj.uid && productObj.uid > 0 && (uid = productObj.uid,
            virtProduct.uid = productObj.uid),
            productObj.recid && productObj.recid > 0 && (recid = productObj.recid,
            virtProduct.recid = productObj.recid),
            productObj.lid && productObj.lid > 0 && (lid = productObj.lid,
            virtProduct.lid = productObj.lid),
            productObj.sku && productObj.sku > 0 && (virtProduct.sku = productObj.sku),
            virtProducts[virtProducts.length] = virtProduct
        }
        var virtProduct;
        "add" !== type && "remove" !== type || (virtPage = "/tilda/cart/" + type + "/",
        recid > 0 && (virtPage += "" + recid),
        uid > 0 ? virtPage += "-u" + uid : lid > 0 && (virtPage += "-" + lid)),
        "detail" === type && (virtPage = "/tilda/product/detail/",
        uid > 0 ? virtPage += uid + "/" : (recid > 0 && (virtPage += "r" + recid),
        lid > 0 && (virtPage += "-l" + lid))),
        "purchase" === type && (virtPage = "/tilda/rec" + recid + "/payment/"),
        (virtProduct = {
            ecommerce: {}
        }).ecommerce[type] = {
    ...
    Tilda.sendEventToStatistics = function(virtPage, virtTitle, referer, price) {
        var isVirtPage = "/" === virtPage.substring(0, 1), products = [], allRec, allRecFbSendEvent;
        ...
        if (currencyCode = allRec.getAttribute("data-tilda-currency") ? allRec.getAttribute("data-tilda-currency") : curBlock && curBlock.getAttribute("data-project-currency-code") ? curBlock.getAttribute("data-project-currency-code") : "RUB",
        referer || (referer = window.location.href),
        (price = price ? parseFloat(price) : 0) > 0)
            if (window.dataLayer || (window.dataLayer = []),
            -1 !== virtPage.indexOf("/tilda/") && -1 !== virtPage.indexOf("/payment/") && window.tildaForm && window.tildaForm.orderIdForStat > "")
                referer = {
                    ecommerce: {
                        purchase: {
                            actionField: {
                ...
                window.tildaForm.tildapayment && window.tildaForm.tildapayment.promocode && (referer.ecommerce.purchase.actionField.coupon = window.tildaForm.tildapayment.promocode),
            ...
            products && products.length > 0)) {
                for (var p = 0; p < products.length; p++)
                    products[p].id || (products[p].sku ? products[p].id = products[p].sku : products[p].uid ? products[p].id = products[p].uid : products[p].recid && products[p].lid && (products[p].id = products[p].recid + "_" + products[p].lid));
                referer.ecommerce.add && referer.ecommerce.add.products ? (referer.ecommerce.add.products = products,
                referer.event = "addToCart") : referer.ecommerce.remove && referer.ecommerce.remove.products ? (referer.ecommerce.remove.products = products,
                referer.event = "removeFromCart") : referer.ecommerce.detail && referer.ecommerce.detail.products ? (referer.ecommerce.detail.products = products,
                referer.event = "viewProduct") : (isVirtPage ? (referer.event = "pageView",
                referer.eventAction = virtPage) : referer.event = virtPage,
                ...
                if (isVirtPage)
                    if (referer && referer.event)
                        if ("purchase" === referer.event) {
                            ...
                            "addToCart" === referer.event && referer.ecommerce.add ? gtag("event", "add_to_cart", {
                        ...
                        !!window.gtagTrackerID && -1 !== window.gtagTrackerID.indexOf("UA-") ? gtag("config", window.gtagTrackerID, {
        ...
        if (window.ga && "tilda" !== window.mainTracker && "gtag" !== window.mainTracker)
            if (isVirtPage)
                if (referer && referer.event) {
                    try {
                        if (window.Tilda.isLoadGAEcommerce || (window.Tilda.isLoadGAEcommerce = !0,
                        ga("require", "ec")),
                        ga("set", "currencyCode", currencyCode),
                        "purchase" === referer.event) {
                            for (var product, iProduct = referer.ecommerce.purchase.products.length, i = 0; i < iProduct; i++)
                            ...
                            for (var product, iProduct = referer.ecommerce.add.products.length, i = 0; i < iProduct; i++)
                            ...
                            for (var product, iProduct = referer.ecommerce.remove.products.length, i = 0; i < iProduct; i++)
                            ...
                            for (var product, iProduct = referer.ecommerce.detail.products.length, i = 0; i < iProduct; i++)
        ...
        if (window.mainMetrikaId && window.mainMetrikaId > 0 && "function" == typeof ym)
            if (isVirtPage) {
                ...
                ym(window.mainMetrikaId, "hit", virtPage, mOpts)) : ym(window.mainMetrikaId, "hit", virtPage, mOpts)
            } else
                price > 0 ? (mOpts = {
                    order_price: price
                },
                currencyCode && (mOpts.currency = currencyCode),
                ym(window.mainMetrikaId, "reachGoal", virtPage, mOpts)) : ym(window.mainMetrikaId, "reachGoal", virtPage);
                    ...
                    if (-1 === virtPage.indexOf("tilda/") || -1 === virtPage.indexOf("/payment/") && -1 === virtPage.indexOf("/submitted/"))
                        if (referer && referer.event && price > 0)
                            if ("addToCart" === referer.event && referer.ecommerce.add) {
                                for (var product, iProduct = referer.ecommerce.add.products.length, content_ids = [], i = 0; i < iProduct; i++)
                                    product = referer.ecommerce.add.products[i],
                                ...
                                for (var product, iProduct = referer.ecommerce.detail.products.length, content_ids = [], i = 0; i < iProduct; i++)
                                    product = referer.ecommerce.detail.products[i],
                                    content_ids.push(product.id || product.uid || product.name);
                        ...
                        price > 0 && currencyCode ? window.fbq("track", "InitiateCheckout", {
                            content_name: virtTitle,
                            content_category: virtPage,
                            value: price,
                            currency: currencyCode
        ...
        if (void 0 !== window.VK && void 0 !== window.VK.Retargeting && !1 === noVkSendEvent)
            try {
                if (isVirtPage) {
                    var allRec, vkPriceListId = (allRec = document.getElementById("allrecords")) ? allRec.getAttribute("data-vk-price-list-id") : null, priceListID = vkPriceListId && parseInt(vkPriceListId) || 0, eventName = "", eventParams = !1, tmp, tmp, tmp, tmp, tmp;
                    if (referer && referer.event)
                        if (eventParams = {
                            products: [],
                            currency_code: "",
                        ...
                        "purchase" === referer.event && referer.ecommerce.purchase)
                            if (price > 0 && priceListID > 0) {
                                ...
                                for (var iProduct = referer.ecommerce.purchase.products.length, content_ids = [], i = 0; i < iProduct; i++)
                                    product = referer.ecommerce.purchase.products[i],
                            ...
                            if (price > 0 && priceListID > 0) {
                                var product;
                                eventParams.currency_code = currencyCode;
                                for (var iProduct = referer.ecommerce.add.products.length, content_ids = [], i = 0; i < iProduct; i++)
                                    product = referer.ecommerce.add.products[i],
                                    eventParams.products.push({
                                        id: product.id || product.uid || product.name,
                                        price: product.price ? product.price : 0
                                ...
                                referer.ecommerce.add[0] && referer.ecommerce.add[0].uid && (eventName += "-" + referer.ecommerce.add[0].uid);
                        else if ("viewProduct" === referer.event && referer.ecommerce.detail)
                            if (price > 0 && priceListID > 0) {
                                var product;
                                eventParams.currency_code = currencyCode;
                                for (var iProduct = referer.ecommerce.detail.products.length, content_ids = [], i = 0; i < iProduct; i++)
                                    product = referer.ecommerce.detail.products[i],
                                    eventParams.products.push({
                                        id: product.id || product.uid || product.name,
                                        price: product.price ? product.price : 0
                                ...
                                referer.ecommerce.detail[0] && referer.ecommerce.detail[0].uid && (eventName += "-" + referer.ecommerce.detail[0].uid);
                        else if ("removeFromCart" === referer.event && referer.ecommerce.remmove)
                            if (price > 0 && priceListID > 0) {
                                var product;
                                eventParams.currency_code = currencyCode;
                                for (var iProduct = referer.ecommerce.remove.products.length, content_ids = [], i = 0; i < iProduct; i++)
                                    product = referer.ecommerce.remove.products[i],
                                    eventParams.products.push({
                                        id: product.id || product.uid || product.name,
                                        price: product.price ? product.price : 0
                                ...
                                referer.ecommerce.remove[0] && referer.ecommerce.remove[0].uid && (eventName += "-" + referer.ecommerce.remove[0].uid);
                        else
                            eventName = referer.event;
                    else if (-1 !== virtPage.indexOf("tilda/") && -1 !== virtPage.indexOf("/payment/"))
                        eventName = "t-purchase-" + (tmp = (tmp = (tmp = virtPage.replace("/tilda/", "")).replace("tilda/", "")).replace("/payment/", ""));
                    else if (-1 !== virtPage.indexOf("tilda/") && -1 !== virtPage.indexOf("/submitted/"))
                        eventName = "t-lead-" + (tmp = (tmp = (tmp = virtPage.replace("/tilda/", "")).replace("tilda/", "")).replace("/submitted/", ""));
                    else if (-1 !== virtPage.indexOf("tilda/popup"))
                        eventName = "t-" + (tmp = (tmp = virtPage.replace("/tilda/", "")).replace("/", "-"));
                    else if (-1 !== virtPage.indexOf("tilda/click"))
                        eventName = "t-" + (tmp = (tmp = virtPage.replace("/tilda/", "")).replace("/", "-"));
                    else
                        eventName = "t-" + (tmp = virtPage.replace("/", "-"));
                    priceListID > 0 && eventParams && eventParams.currency_code ? (VK.Retargeting.Event("purchase"),
                    VK.Retargeting.ProductEvent(priceListID, eventName, eventParams)) : (VK.Retargeting.Event(eventName),
                    "t-purchase" === eventName.substring(0, 10) ? VK.Goal("purchase") : "t-lead" === eventName.substring(0, 6) && VK.Goal("lead"))
                } else
        ...
        "function" == typeof window.tildastat && (isVirtPage ? (virtPage.indexOf("payment") > 0 && virtPage.indexOf("tilda/form") > -1 && (virtPage = virtPage.replace("tilda/form", "tilda/rec")),
        ...
        try {
            ...
            if (-1 !== tildaPageURL.toLowerCase().indexOf("utm_") && "string" == typeof (tildaPageQuery = (tildaPageQuery = (tildaPageURL = tildaPageURL.toLowerCase()).split("?"))[1])) {
                ...
                for (var i in arParams)
                    "amp;" === (arPair = arParams[i].split("="))[0].substring(0, 4) && (arPair[0] = arPair[0].substring(4)),
                    "utm_" === arPair[0].substring(0, 4) && (tildaPageUTM = tildaPageUTM + arParams[i] + "|||");
