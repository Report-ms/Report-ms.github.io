function t_appendGoogleMap(recid, key) {
    if ("object" == typeof google && "object" == typeof google.maps)
        ...
        tildaMapElement && (mapLang = tildaMapElLang || ""),
...
function t_handleGoogleApiReady(recid) {
    ...
    Array.prototype.forEach.call(gmaps, (function(gmap) {
        ...
        Array.prototype.forEach.call(arMarkers, (function(el) {
            ...
            (el.descr || el.title) && t_map__attachInfoMessage(marker, el.descr, el.title, el.isOpen)
        }
        ));
        var mapPathAttr = gmap.getAttribute("data-map-path"), mapPathColorAttr = gmap.getAttribute("data-map-path-color"), mapPathWeightAttr = gmap.getAttribute("data-map-path-weight"), path;
        ...
        function t_map__attachInfoMessage(marker, descr, title, isOpen) {
            ...
            var baloonTitle, baloonDescr, baloonContent = [title ? "<b>" + title + "</b>" : "", textarea.textContent].filter(Boolean).join("<br>"), infowindow = new google.maps.InfoWindow({
                content: baloonContent
            });
            isOpen && infowindow.open(marker.get("map"), marker),
        ...
        if (arMarkers.length > 1) {
            ...
            var listener = google.maps.event.addListener(map, "idle", (function() {
                (map.getZoom() > parseInt(mapZoomAttr, 10) || 0 === map.getZoom()) && map.setZoom(parseInt(mapZoomAttr, 10)),
        ...
        google.maps.event.addDomListener(window, "resize", (function() {
            ...
            arMarkers.length > 0 && (map.fitBounds(bounds),
            zoom > 0 && (map.getZoom() > zoom || 0 == map.getZoom()) && map.setZoom(zoom))
        }
        )),
...
function t_handleYandexApiReady(recid) {
    ...
    Array.prototype.forEach.call(ymapsArr, (function(ymap) {
        ...
        window.isDragMap = !window.isMobile;
        var myLatLng = arMarkers.length > 0 && [parseFloat(arMarkers[0].lat), parseFloat(arMarkers[0].lng)], myOptions = {
            zoom: parseInt(mapZoomAttr, 10),
            center: myLatLng,
            scrollZoom: !1,
            controls: ["typeSelector", "zoomControl"],
            drag: window.isDragMap
        }, map = new ymaps.Map(ymap,myOptions), eventsPane, eventsPaneEl = map.panes.get("events").getElement(), mobilePanelText = {
        ...
        window.isMobile && (map.behaviors.disable("drag"),
        ...
        var placemarks = arMarkers.map((function(marker) {
            ...
            var baloonTitle, baloonDescr, baloonContent = [marker.title ? "<b>" + marker.title + "</b>" : "", textarea.textContent].filter(Boolean).join("<br>"), placemark;
        ...
        if (mapModeAttr) {
            ...
            if (groundPaneElementChild && (groundPaneElementChild.style.filter = "bw_light" === mapModeAttr ? "grayscale(1)" : "grayscale(1) invert(1)"),
            mapColorAttr) {
                var groundPaneElement = ymap.querySelector('ymaps[class$="ground-pane" i]')
                  , coloredDiv = document.createElement("div");
                if (coloredDiv.style.opacity = ".15",
        ...
        zoom > 0 && (0 == map.getZoom() || map.getZoom() > zoom) && map.setZoom(zoom)) : zoom > 0 && (0 == map.getZoom() || map.getZoom() > zoom) && map.setZoom(zoom),
...
function t_map_hexToHsl(H) {
    var r = 0
      , g = 0
      , b = 0;
    4 === H.length ? (r = "0x" + H[1] + H[1],
    g = "0x" + H[2] + H[2],
    b = "0x" + H[3] + H[3]) : 7 === H.length && (r = "0x" + H[1] + H[2],
    g = "0x" + H[3] + H[4],
    b = "0x" + H[5] + H[6]),
    r /= 255,
    g /= 255,
    ...
    l = (cmax + cmin) / 2,
    [h, s = +(100 * (s = 0 === delta ? 0 : delta / (1 - Math.abs(2 * l - 1)))).toFixed(1), l = +(100 * l).toFixed(1)]
}
